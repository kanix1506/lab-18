...
student text2student(string text){
    ...
    s.name = name;
    s.id = atoi(id.c_str());    // แปลง string เป็น int
    s.gender = gen[0];          // ดึงตัวอักษรแรก
    s.gpa = atof(gpa.c_str());  // แปลง string เป็น float
    
    return s;
}

// ต้องเติม & หลัง vector<student> เพื่อรับค่าแบบ Reference
	...
	while(getline(course_file,textline)){
		if(state == 1){
			...
			if(textline == "---------------------------------------"){
				...
				if(p != 0) { // ตรวจสอบว่าหาเจอหรือไม่ก่อน push
